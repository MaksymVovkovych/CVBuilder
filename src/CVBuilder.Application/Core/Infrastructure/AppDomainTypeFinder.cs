...
public class AppDomainTypeFinder : ITypeFinder
{
    ...
    public AppDomainTypeFinder(ICVBuilderFileProvider fileProvider = null)
    {
    ...
    protected ICVBuilderFileProvider FileProvider;
    ...
    protected virtual void LoadMatchingAssemblies(string directoryPath)
    {
        ...
        foreach (var a in GetAssemblies()) loadedAssemblyNames.Add(a.FullName);

        if (!FileProvider.DirectoryExists(directoryPath)) return;

        foreach (var dllPath in FileProvider.GetFiles(directoryPath, "*.dll"))
            try
            {
                var an = AssemblyName.GetAssemblyName(dllPath);
                if (Matches(an.FullName) && !loadedAssemblyNames.Contains(an.FullName)) App.Load(an);
            }
